 ...
 */
Datum
gp_switch_xlog(PG_FUNCTION_ARGS)
{
	typedef struct Context
	{
		CdbPgResults cdb_pgresults;
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		TupleDescInitEntry(tupdesc, (AttrNumber) 2, "switch_lsn", LSNOID, -1, 0);
		...
		if (!IS_QUERY_DISPATCHER())
			elog(ERROR, "cannot use pg_switch_xlog() when not in QD mode");

		restore_command =
				psprintf("SELECT pg_catalog.pg_switch_xlog()");
		...
		SIMPLE_FAULT_INJECTOR("gp_switch_xlog_acquired_lock");
